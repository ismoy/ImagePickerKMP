...
internal object OCRUtils {
    ...
    fun detectMimeType(fileData: ByteArray, fileName: String?): String {
        return when {
            fileData.size >= 4 && fileData.sliceArray(0..3)
                .contentEquals(byteArrayOf(0x25, 0x50, 0x44, 0x46)) -> "application/pdf"
            fileData.size >= 8 && fileData.sliceArray(0..7)
                .contentEquals(byteArrayOf(0x89.toByte(), 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A)) -> "image/png"
            fileData.size >= 3 && fileData[0] == 0xFF.toByte()
                    && fileData[1] == 0xD8.toByte() && fileData[2] == 0xFF.toByte() -> "image/jpeg"
            fileData.size >= 12 && fileData.sliceArray(8..11).decodeToString() == "WEBP" -> "image/webp"
            fileData.size >= 6 && (fileData.sliceArray(0..5).decodeToString() == "GIF87a"
                    || fileData.sliceArray(0..5).decodeToString() == "GIF89a") -> "image/gif"
