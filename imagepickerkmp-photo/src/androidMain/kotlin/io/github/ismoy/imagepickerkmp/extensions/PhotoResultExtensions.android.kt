package io.github.ismoy.imagepickerkmp.extensions
...
private fun loadBytesFromURI(uri: String, compressionLevel: CompressionLevel, cacheKey: String?): ByteArray {
    ...
    return try {
        ...
        val bitmap = if (uri.startsWith("/") || uri.startsWith("file://")) {
            ...
            options.inSampleSize = calculateOptimalSampleSize(options, compressionLevel.toMaxDimension())
            ...
            context.contentResolver.openInputStream(uriObj)?.use { inputStream ->
                val options = BitmapFactory.Options().apply {
                    inSampleSize = calculateOptimalSampleSize(this, compressionLevel.toMaxDimension())
                    inPreferredConfig = Bitmap.Config.RGB_565
        ...
        bitmap.compress(Bitmap.CompressFormat.JPEG, compressionLevel.toJpegQuality(), outputStream)
        bitmap.recycle()
...
private fun loadOptimizedBytesFromURI(uri: String, cacheKey: String): ByteArray {
    return loadBytesFromURI(uri, CompressionLevel.MEDIUM, cacheKey)
}
...
private fun loadOptimizedImageBitmapFromURI(uri: String, cacheKey: String): ImageBitmap? {
    ...
    return runCatching {
        ...
        context.contentResolver.openInputStream(uriObj)?.use { inputStream ->
            val options = BitmapFactory.Options().apply {
                inSampleSize = calculateOptimalSampleSize(this, 1280)
                inPreferredConfig = Bitmap.Config.RGB_565
                inJustDecodeBounds = false
            }
...
// Removed unused methods optimizeImageBytesWithQuality and processImageWithCustomCompression
...
private fun getApplicationContext(): Context {
    ...
    cachedAppContext = WeakReference(ctx)
    return ctx
}
