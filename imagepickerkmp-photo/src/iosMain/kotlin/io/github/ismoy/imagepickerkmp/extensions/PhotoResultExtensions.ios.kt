        ...
        ByteArray(compressedData.length.toInt()).apply {
            usePinned { pinned ->
                compressedData.bytes?.let { bytes ->
                    memcpy(pinned.addressOf(0), bytes, compressedData.length)
                }
            }
        }
    } catch (_: Exception) {
...
@OptIn(ExperimentalForeignApi::class)
private fun resizeImageIOS(image: UIImage, newSize: CValue<CGSize>): UIImage {
...
private val coreFileSystem = CoreServices.fileSystemManager(subDirectory = "imagepicker_temp")

private val cacheDirectoryPath: String by lazy {
    runBlocking { coreFileSystem.getCacheDirectory().path }
}
...
actual val PhotoResult.absolutePath: String
    get() {
        ...
        val destURL = NSURL.fileURLWithPath(cacheDirectoryPath)
